...
export async function checkBurnedAndLockedAndBuy(id: PublicKey, poolState: LiquidityStateV4) {
  if(CHECK_IF_IS_BURNED || CHECK_IF_IS_LOCKED){
    ...
    if(CHECK_IF_IS_BURNED){
      if(burned != true){
        threadSize = threadSize - 1
        logger.warn(`Skipped, liquidity not burned, mint:${poolState.baseMint}, threadSize: ${threadSize}`);
        return
      }
    }
    ...
    if(CHECK_IF_IS_LOCKED){
      if(locked != true){
        threadSize = threadSize - 1
        logger.warn(`Skipped, liquidity not locked, mint:${poolState.baseMint}, threadSize: ${threadSize}`);
        return
      }
    }   
    threadSize = threadSize - 1
